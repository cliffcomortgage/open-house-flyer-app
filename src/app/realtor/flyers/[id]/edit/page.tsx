...
import type { PropertyData, Flyer } from "@/types";
...
function TemplateStep({ selected, onSelect }: { selected: string; onSelect: (id: string) => void }) {
  const layoutIcons: Record<string, React.ReactNode> = {
    single: <div className="w-full h-16 bg-slate-100 rounded-md" />,
    grid: (
...
export default function EditRealtorFlyerPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`/api/realtor-portal/flyers/${id}`);
        if (!res.ok) throw new Error();
        const flyer: Flyer = await res.json();
        setTemplateId(flyer.templateId);
        setPropertyData(flyer.propertyData || { photos: [] });
  ...
  const handleSave = async (status: "DRAFT" | "SAVED") => {
    ...
    try {
      ...
      if (status === "SAVED") {
        router.push(`/realtor/flyers/${id}/preview`);
      }
    } catch (err: any) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">Update your listing flyer details.</p>
      </div>
