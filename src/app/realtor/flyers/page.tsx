...
function FlyerCard({ flyer, onDelete }: { flyer: Flyer; onDelete: () => void }) {
  ...
  const handleDownloadPDF = async () => {
    setDownloading(true);
    try {
      const res = await fetch(`/api/realtor-portal/flyers/${flyer.id}/pdf`, { method: "POST" });
      if (!res.ok) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-sm transition-shadow group">
      <div className="h-36 relative flex items-center justify-center" style={{ backgroundColor: "#6633cc" }}>
        {propertyData?.photos?.[0] ? (
          <img src={propertyData.photos[0]} alt="Property" className="w-full h-full object-cover" />
        ) : (
        ...
        <div className="absolute top-2 right-2">
          <Badge className={flyer.status === "SAVED" ? "bg-emerald-500 text-white border-0" : "bg-amber-500 text-white border-0"}>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1 mt-3 pt-3 border-t border-slate-100">
          ...
          {flyer.shareToken && (
            <Button variant="ghost" size="icon" className="h-8 w-8" title="Copy share link" onClick={handleCopyShare}>
...
export default function RealtorFlyersPage() {
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`/api/realtor-portal/flyers/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="text-center py-24">
          ...
          <p className="text-sm text-slate-400 mb-6">
            {statusFilter !== "ALL"
              ...
              : "Create your first listing flyer to get started."}
          </p>
          {statusFilter === "ALL" && (
            <Button asChild style={{ backgroundColor: "#6633cc" }} className="text-white">
              <Link href="/realtor/flyers/new">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((flyer) => (
            <FlyerCard key={flyer.id} flyer={flyer} onDelete={() => handleDelete(flyer.id)} />
          ))}
        </div>
      )}
    </div>
